...
import copy
# from arm import Arm
from geometry import does_alien_touch_goal, does_alien_touch_wall, is_alien_within_window
from util import configToIdx
...
def transformToMaze(alien, goals, walls, window, granularity):
    ...
    # START POINT
    a,b = alien.get_centroid() 

    numRow = int(window[1] / granularity + 1)
    numCol = int(window[0] / granularity + 1)

    myMaze = [[[SPACE_CHAR for z in range(3)] for y in range(numRow)] for x in range(numCol)]

    loc = configToIdx((a, b, alien.get_shape()), [0, 0, 0], granularity, alien)
    
    
    for k in range(3):
        for i in range(numRow):
            for j in range(numCol):
                config1 = idxToConfig((j,i,k), [0,0,0], granularity, alien)
                alien.set_alien_config(config1) 
            
                if (does_alien_touch_wall(alien, walls, granularity)):
                    myMaze[j][i][k] = WALL_CHAR
        
                elif (is_alien_within_window(alien, window, granularity) == False):
                    myMaze[j][i][k] = WALL_CHAR
                    
                elif (does_alien_touch_goal(alien, goals)):
                    myMaze[j][i][k] = OBJECTIVE_CHAR
                    
                else:
                    myMaze[j][i][k] = SPACE_CHAR

    # START POINT
    myMaze[loc[0]][loc[1]][loc[2]] = START_CHAR
    mazeCreated = Maze(myMaze, alien, granularity, [0, 0, 0], None)
